        ...
        if(message.mentions.users.first()){
            if(message.content.split(" ")[2]){
                con.query(`SELECT * FROM profiles WHERE user_id = ${message.mentions.users.first().id}`,(err,row) => {
                    con.query(`UPDATE profiles SET user_lvl = ${message.content.split(" ")[2]} WHERE user_id = ${message.mentions.users.first().id}`)
                ...
                .then(u => { 
                    if(message.content.split(" ")[2]){
                        con.query(`SELECT * FROM profiles WHERE user_id = ${u.id}`,(err,row) => {
                            con.query(`UPDATE profiles SET user_lvl = ${message.content.split(" ")[2]} WHERE user_id = ${u.id}`)
...
module.exports.help = async (bot,message,transsv,nickcmds,db) => {
    ...
    let embed = new Discord.RichEmbed ()
        ...
        .setDescription(`${transsv.setlvl.help.description} ${botowner.username}#${botowner.discriminator} <@${botowner.id}>`)
        .addField(transsv.use,"```"+transsv.setlvl.help.use+"```",true)
        .addField(transsv.setlvl.help.before,transsv.setlvl.help.btext)
